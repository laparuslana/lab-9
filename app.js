...
app.get('/', (req, res) => {
    ...
    if (req.session.username) {
        fluentd.emit('access', { message: 'User accessed homepage', session: req.session });
        return res.json({
...
app.get('/logout', (req, res) => {
    fluentd.emit('access', { message: 'User logged out', session: req.session });
...
app.post('/api/login', (req, res) => {
    ...
    if (user) {
        ...
        fluentd.emit('login', { message: 'User logged in', username: user.username });
        return res.json({ username: login });
    }

    fluentd.emit('login', { message: 'Login attempt failed', login: login });
    res.status(401).send();
});
